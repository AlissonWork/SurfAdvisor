...
export default function SmartMatch() {
  ...
  async function getRecommendation() {
    ...
    try {
      const response = await api.get(`/boards/recomendacao?pico=${selectedSpot}`);
      setRecommendation(response.data);
    } catch (err) {
      console.error(err);
      setError(err.response?.data?.message || "Error fetching recommendation. Please try again.");
    } finally {
      setLoading(false);
    }
  }
      ...
      <main className="p-6 md:p-10 max-w-6xl mx-auto mt-8">
        ...
        <div className="text-center mb-12">
          ...
          <h1 className="text-4xl md:text-5xl font-bold text-white mb-4">
            Smart <span className="text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 to-teal-400">Match</span>
          </h1>
          <p className="text-slate-400 text-lg max-w-2xl mx-auto">
            Select your surf spot. We analyze real-time ocean data and your skill level to pick the perfect board from your quiver.
          </p>
        </div>
        ...
        <div className="bg-slate-800/40 border border-slate-700/50 backdrop-blur-md rounded-3xl p-6 md:p-8 shadow-2xl mb-8">
          <div className="flex flex-col md:flex-row gap-4 items-center justify-center">
            <div className="w-full md:w-2/3">
              <label className="block text-sm font-medium text-slate-400 mb-2 ml-1">Select Spot</label>
              <select 
                value={selectedSpot} 
            ...
            <div className="bg-gradient-to-b from-cyan-950/40 to-slate-900/40 border border-cyan-900/50 rounded-3xl p-6 md:p-8 animate-fade-in-up">
              ...
              <div className="flex flex-col md:flex-row justify-between items-center mb-8 pb-6 border-b border-slate-700/50 gap-4">
                <div className="text-center md:text-left">
                  <p className="text-slate-400 text-sm uppercase tracking-widest mb-1">Current Condition at {recommendation.local || "Spot"}</p>
                  <p className="text-3xl font-bold text-white flex items-center justify-center md:justify-start gap-2">
                    <span className="text-cyan-400 text-4xl">≈</span> {recommendation.condicaoMar}
                  </p>
                </div>
                <div className="text-center md:text-right">
                  <p className="text-slate-400 text-sm uppercase tracking-widest mb-1">Surfer Profile</p>
                  <p className="text-white font-medium capitalize">{recommendation.surfista} • {recommendation.nivel} • {recommendation.peso}kg</p>
                </div>
              </div>
              ...
              {recommendation.suggestions.length > 0 ? (
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  {recommendation.suggestions.map((board) => (
                    <div key={board._id} className="bg-slate-800/80 border border-slate-600/50 rounded-2xl p-4 hover:border-cyan-500/50 transition-all flex gap-4 items-center group cursor-default">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex justify-between items-start mb-1">
                          <h4 className="text-base sm:text-lg font-bold text-white leading-tight truncate pr-2" title={board.nome}>
                            {board.nome}
                          </h4>
                        </div>
                        ...
                        <div className="text-slate-300 text-xs sm:text-sm space-y-0.5">
                          <p>Size: <span className="text-white font-medium">{board.tamanho}</span></p>
                          <p>Volume: <span className="text-white font-medium">{board.litragem}L</span></p>
                ...
                <div className="bg-slate-800/50 border border-orange-500/20 rounded-2xl p-8 text-center">
                  <p className="text-orange-400 text-lg mb-2">No perfect match found!</p>
                  <p className="text-slate-400 text-sm">
                    Based on your {recommendation.peso}kg weight and {recommendation.nivel} skill level, none of your boards are ideal for {recommendation.condicaoMar} waves. Time to buy a new one?
                  </p>
                </div>
              )}
            </div>
        ...
        <section className="mt-8">
          <div className="text-center mb-10">
            <h2 className="text-3xl font-bold text-white mb-3">
              Board <span className="text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 to-teal-400">Styles Guide</span>
            </h2>
            <p className="text-slate-400 max-w-xl mx-auto">
              Learn about each board style and find the perfect match for your skill level and wave conditions.
            </p>
          </div>
